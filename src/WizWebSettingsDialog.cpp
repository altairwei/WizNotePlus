...
WizWebSettingsDialog::WizWebSettingsDialog(const WizWebEngineInjectObjectCollection& objects, QString url, QSize sz, QWidget *parent )
...
void WizWebSettingsDialog::init(const WizWebEngineInjectObjectCollection &objects, QSize sz, QWidget *parent)
{
    ...
    if (mainWindow) {
        WizWebEngineInjectObjectCollection tmp = objects;
        tmp.insert("WizExplorerApp", mainWindow->publicAPIsObject());
        auto profile = createWebEngineProfile(tmp, this);
        auto webPage = new WizWebEnginePage(tmp, profile, web);
        web->setPage(webPage);
    }
