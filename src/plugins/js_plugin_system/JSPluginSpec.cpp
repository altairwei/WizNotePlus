    ...
    , m_path(path)
{
    ...
    for (QString& pluginIndex : groups) {
        if (!pluginIndex.contains("Module_"))
            continue;
        JSPluginModuleSpec* data = new JSPluginModuleSpec(pluginIndex, *m_settings, this);
...
//
void JSPluginSpec::emitDocumentChanged()
{
    emit documentChanged();
    for (JSPluginModuleSpec* data : m_modules) {
        data->emitDocumentChanged();
    }
}

void JSPluginSpec::emitShowEvent()
{
    emit willShow();
    for (JSPluginModuleSpec* data : m_modules) {
        data->emitShowEvent();
...
JSPluginModuleSpec::JSPluginModuleSpec(QString& section, WizSettings& setting, QObject* parent)
    : QObject(parent)
    , m_section(section)
{
    m_parentPlugin = qobject_cast<JSPluginSpec*>(parent);
...
void JSPluginModuleSpec::emitDocumentChanged()
{
    emit documentChanged();
}

void JSPluginModuleSpec::emitShowEvent()
{
    emit willShow();
}