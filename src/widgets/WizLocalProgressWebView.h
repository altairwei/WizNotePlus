...
public:
    ...
    explicit WizLocalProgressWebView(const WizWebEngineInjectObjectCollection &objects, QWidget *parent = 0);
    ~WizLocalProgressWebView();

    WizWebEngineView* web();

private:
    void init(const WizWebEngineInjectObjectCollection &objects);
